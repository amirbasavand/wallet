...
const Dashbord = (props) => {
    ...
    const loadDataFromDataBase = useCallback(async () => {
        let response = localStorage.getItem('contacts') ? JSON.parse(localStorage.getItem('contacts')) : []
        if (response.length > 0) {
            let responseBody = response.filter(item => item.userId === auth.userId).filter(item => {
                return item.name.includes(search)
            })
            setContacts(responseBody)
        }
    ...
    const addContact = async (e) => {
        ...
        if (!editFalse) {
            if (isValid()) {
                ...
                let items = localStorage.getItem('contacts') ? JSON.parse(localStorage.getItem('contacts')) : []
        ...
        if (editFalse) {
            let response = localStorage.getItem('contacts') ? JSON.parse(localStorage.getItem('contacts')) : []
            if (response.length > 0) {
                let responseBody = response.map(item => {
                    if (item.id === editId) {
                        return {
                            id: editId,
                            userId: auth.userId,
                ...
                localStorage.setItem('contacts', JSON.stringify(responseBody))
                setContacts(responseBody)
                setState({ ...state, name: '', number: '' })
    ...
    const deleteProfile = async (id) => {
        let response = localStorage.getItem('contacts') ? JSON.parse(localStorage.getItem('contacts')) : []
        if (response.length > 0) {
            let responseBody = response.filter(item => item.id !== id)
            localStorage.setItem('contacts', JSON.stringify(responseBody))
            setContacts(responseBody)
            propContact.classList.remove('active')
            loadDataFromDataBase()
    ...
    return (
        <div className='container'>
            <div className="sidebar">
                <Link to='/dashbord/user' className='color_link' onClick={removeActive}>
                ...
                <div className="sidebar_top">
                    <div className="sidebar_item item1" onClick={(e) => addStyle(e)}>
                        <Link to='/dashbord/transaction' className='color_link'> <AiOutlineHome />{' '} <span> تراکنش ها </span></Link>
                    </div>
                    <div className="sidebar_item item2" onClick={(e) => addStyle(e)}>
                        <Link to='/dashbord/contact/s' className='color_link'>     <MdContactPhone /> {' '} <span>مخاطبین </span></Link>
                    </div>
                    <div className="sidebar_item item3" onClick={(e) => addStyle(e)}>
                        <Link to='/dashbord/user/more' className='color_link'>   <AiFillProfile /> {' '}<span>موارد بیشتر </span></Link>
                    </div>
            ...
            <Switch>
                <Route exact path='/dashbord/contact/:id'>
            ...
            <Addcontact removeActive={removeActive} state={state} setState={setState} addContact={addContact} />
